...
exports.handler = async (event) => {
  ...
  try {
    ...
    // If no key yet, return a simple, non-AI plan so the UI still shows something.
    if (!OPENAI_API_KEY) {
      return {
        statusCode: 200,
        ...
        body: JSON.stringify({
          ...
          notes: 'Demo plan: add OPENAI_API_KEY for live AI.'
        })
      };
    }

    // Build your prompt (uses all onboarding fields)
    const profile = body;
    const prompt = `
...
Rules: professional-grade training (6–7 movements); respect injuries & equipment; match diet style & allergies; adhere to calorie_target ±10%. Return ONLY JSON.
`;
    ...
    const resp = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        model: 'gpt-5.1-mini',   // use a model your account has
        input: prompt,
        temperature: 0.7,
